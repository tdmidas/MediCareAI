    ...
    useEffect(() => {
        const fetchAndUpdateHealthData = async () => {
            try {
                ...
                const bpResponse = await axios.get(`https://tdmidas.id.vn/api/health/bloodPressure/${userId}`);
                const { diaBP, sysBP } = bpResponse.data;

                const glucoseResponse = await axios.get(`https://tdmidas.id.vn/api/health/glucose/${userId}`);
